...
#include "com_vulkan_compute_demo_VulkanComputeDemoJni.h"

#define LOG_TAG "VulkanComputeDemoJni-CPP"
...
Java_com_vulkan_compute_demo_MainActivity_nativeVulkanComputeDemoJni(JNIEnv *env,
    ...
    DPRINTF("nativeVulkanComputeDemoJni enter");
    char spv_buffer[1024 * 8]; //8k byte buffer
    jint spv_size = env->GetArrayLength(spvByteArray);
    if (spv_size < sizeof(spv_buffer)) {
        jbyte *spv_data = env->GetByteArrayElements(spvByteArray, 0);
        if (NULL != spv_data) {
            memset(spv_buffer, 0x00, sizeof(spv_buffer));
            memcpy(spv_buffer, spv_data, (int) spv_size);
        }
        double time_ms = now_ms();
        VkResult result = vulkanVecorAdditionComputePerform(spv_size, spv_buffer);
        time_ms = now_ms() - time_ms;
        DPRINTF("vulkanVecorAdditionComputePerform consume %fms", time_ms);

        env->ReleaseByteArrayElements(spvByteArray, spv_data, 0);

        if (VK_SUCCESS == result)
            return env->NewStringUTF(("Vulkan Compute Demo run successfully \n"
                                              "Consume : " +
                                      std::to_string(time_ms) + "ms").c_str());
        else
            return env->NewStringUTF(("Vulkan Compute Demo run failure, reason:" +
                                      std::to_string(result)).c_str());
    }
    return env->NewStringUTF("SPV content size exceed cahce 8k size");
}



VkResult vulkanVecorAdditionComputePerform(int spvArraySize, char *spvArray) {
    DPRINTF("vulkanVecorAdditionComputePerform enter");
    ...
    error = vkEnumeratePhysicalDevices(instance, &deviceCount, nullptr);
    if (VK_SUCCESS != error) {
        return error;
    }
    ...
    error = vkEnumeratePhysicalDevices(instance, &deviceCount, physicalDevices);
    if (VK_SUCCESS != error) {
        return error;
    }
    ...
    for (uint32_t i = 0; i < deviceCount; i++) {
        ...
        std::vector<VkQueueFamilyProperties> queueFamilyProperties(deviceCount);
        vkGetPhysicalDeviceQueueFamilyProperties(device, &deviceCount,
    ...
    vkGetPhysicalDeviceProperties(physicalDevice, &phy_device_prop);
    DPRINTF("Physical Vulkan Device Name='%s' Physical Vulkan Device ID=%#010x",
            phy_device_prop.deviceName, physicalDevice);
    ...
    VkDevice device;
    error = vkCreateDevice(physicalDevice, &deviceCreateInfo, 0, &device);
    if (VK_SUCCESS != error) {
        return error;
    }
    ...
    shaderModuleCreateInfo.pCode = reinterpret_cast<uint32_t *>(spvArray);
    shaderModuleCreateInfo.codeSize = spvArraySize;
    ...
    for (int32_t index = 0; index < memoryProperties.memoryTypeCount; ++index) {
        VkMemoryType memoryTypes = memoryProperties.memoryTypes[index];
        if (bufferAMemoryRequirements.memoryTypeBits & (1 << index) &&
            (VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT & memoryTypes.propertyFlags) &&
            (VK_MEMORY_PROPERTY_HOST_COHERENT_BIT & memoryTypes.propertyFlags)) {
            memoryTypeIndex = index;
            break;
        }
    }
    if (VK_MAX_MEMORY_TYPES == memoryTypeIndex) {
    ...
    //vector addition validation
    for (uint32_t index = 0; index < elements; index++) {
        if (DBG_FLAG)
            DPRINTF("resultData[%d]=%u, aData[%d]=%u, bData[%d]=%u",
                    index, resultData[index], index, aData[index], index, bData[index]);
        if (resultData[index] != aData[index] + bData[index]) {
            EPRINTF("result[%d]='%d' is not '%d!'", index, resultData[index],
